import os
import argparse
from model_bispcr import MODEL
import torch
...
from data_bispcr import ModelNet40
...
def eval_one_epoch(args, net, test_loader):
    net.eval()
...
def test_one_epoch(args, net, test_loader, epoch):
    ...
    write_metrics_for_reg(os.path.join('./log/', "bispcr_unseen_noise.txt"), epoch=epoch, train=1, r_rmse=r_rmse, r_mae=r_mae, t_rmse=t_rmse, t_mae=t_mae)
...
def train(args, net, train_loader, test_loader):
    ...
    for epoch in range(args.epochs):
        ...
        train_stats = train_one_epoch(args, net, train_loader, opt, epoch)
        test_stats = test_one_epoch(args, net, test_loader, epoch)
...
def main():
    ...
    os.system('cp main_bispcr.py checkpoints' + '/' + args.exp_name + '/' + 'main_bispcr.py.backup')
    os.system('cp model_bispcr.py checkpoints' + '/' + args.exp_name + '/' + 'model_bispcr.py.backup')
    ##### make checkpoint directory and backup #####


    if args.dataset == 'modelnet40':
        train_loader = DataLoader(
            ModelNet40(partition='train', alpha=args.alpha, gaussian_noise=args.gaussian_noise, unseen=args.unseen, factor=args.factor, clip=args.clip),
            batch_size=args.batch_size, shuffle=True, drop_last=True, num_workers=8)
        test_loader = DataLoader(
            ModelNet40(partition='test', alpha=args.alpha, gaussian_noise=args.gaussian_noise, unseen=args.unseen, factor=args.factor, clip=args.clip),
