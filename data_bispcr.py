...
def fix_Rt(angle):
    ...
    Rz = np.array([[cosz, -sinz, 0],
                    ...
                    [0, 0, 1]])
    R = Rx.dot(Ry).dot(Rz)
    t = np.random.uniform(0, 0, size=3)
    euler = np.array([anglez, angley, anglex])
...
class ModelNet40(Dataset):
    def __init__(self, num_points=1024, partition='train', gaussian_noise=False, alpha=0.75, unseen=False, factor=32, clip=0.05):
